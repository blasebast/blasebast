...
from blasebast.smoothLogging import smoothLogging

lobject = smoothLogging()
...
def check_age(path):
    try:
        ...
        log.error("cannot stat the file '%s', access denied?" % path)
        return 0
                ...
                if str(dir).__contains__(str(dirPattern)):
                    ...
                    if check_age(dirPath) >= age:
                        ...
                        try:
                            ...
                            if move_only is ".":
                                log.info("deleting %s, age is: %s days" % (os.path.join(path,dir),itsAge))
                                shutil.rmtree(target)
                            else:
                                if not os.path.exists(move_only):
                                    try: os.makedirs(path)
                                    except: log.error("cannot create path: %s" % (move_only))
                                log.info("moving %s, age is: %s days [dest: %s]" % (os.path.join(path,dir),itsAge,move_only))
                ...
                if file.endswith(extension):
                    ...
                    if check_age(filepath) >= age and filepath != log_file_name:
                        try:
                            ...
                            if move_only is ".":
                                log.info("removing %s, age is: %s days" % (filepath,itsAge))
                                os.unlink(filepath)
                            else:
                                if not os.path.exists(move_only):
                                    try: os.makedirs(path)
                                    except: log.error("cannot create path: %s" % (move_only))
                                log.info("moving %s, age is: %s days [dest: %s]" % (filepath,itsAge, move_only))
                                shutil.move(filepath, move_only)
                        except:
                            log.error("cannot move / delete %s, but proceeding with rest" % filepath)
...
def create_scheduled_task(recycleWeekDay, recycleWeekDayTime, executableFile, taskName):
    ...
    log.info("command for removing scheduled task is: %s" % removeScheduledTaskCmd)
    log.info("command for adding scheduled task is: %s" % addScheduledTaskCmd)
...
try:
    for resource, settings in yamlStream['removeOldFiles']['c_drive'].items():
        try:
            ...
            log.error("yaml structure is not as expected, it should contain path, extension and age in it's structure" % (yamlFile))
        for extension in extensions.split(','):
            delete_files_by_age(extension,age,path, move_only)
            try:
                delete_files_by_age(extension,age,path, move_only)
            except:
                log.error("cannot delete extension %s on %s" % (extension,path))
        try:
...
if args.createTask == "yes" and os.path.exists(os.path.normpath(python)):
    try:
        ...
        log.warn("incorrectly enabled task?")
    try:
        ...
        if not os.path.exists(dst):
            ...
            log.info("copied yaml lib to %s" % dst)
    except:
        log.error("cannot copy %s to %s" % (src,dst))
else:
    if not os.path.exists(python):
        log.error("%s doesn't exist" % python)
